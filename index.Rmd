...
```{r contraceptive_use, echo=FALSE}
library("tidyverse")
library("dplyr")
...
comparison <- ggplot(data = sex_education,
  ...
  geom_boxplot()
```
Contraceptive plot: Every state uses different contraceptives to different extents, while some states have very few people that use them. To analyze the use of. these contraseptives, we created a boxplot showing a summary of the total contraseptives use in all states. From this data summary we can see that there are states where about 89.9% of the sexually active population use contraseptives, while there also being a state where about only 71.1% of the sexually active population use contraseptives. The median rests at about 82.2% showing us that majority of the states sexually active population use contraceptives. However, there are 
